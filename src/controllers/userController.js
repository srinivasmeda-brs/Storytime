...
const createUser = async (req, res, next) => {
  ...
  try {
    ...
    const verificationResponse = await sendMailVerificationLink(
      email,
      token,
      first_name
    );
...
const verifyEmail = async (req, res, next) => {
  try {
    ...
    if (user.verify_token_expires <= Date.now()) {
      ...
      return res
        .status(201)
        .send("Email is verified successfully. You can now login.");
    }
  } catch (err) {
    next(err); // Pass error to the error handler
  }
...
const loginUser = async (req, res, next) => {
  ...
  try {
    ...
    try {
      ...
      return res
        .status(500)
        .json({ message: "Failed to fetch Spotify token", error: err.message });
    }
...
const resetPassword = async (req, res, next) => {
  ...
  try {
    ...
    if (!user) {
      const err = new Error(
        "Token expired or invalid. Please generate a new one."
      );
      err.status = 401;
      return next(err);
    }
